...
func NewModulesFile(path string, dir string) (mf *ModulesFile, err error) {
	if !filepath.IsAbs(dir) {
		...
		if err != nil {
			err = errors.Warning("forg: new modules file failed").WithCause(err).WithMeta("dir", dir)
			return
		}
	}
...
func (mf *ModulesFile) writeServices(ctx context.Context) (err error) {
	// services
	const (
		exports = `// NOTE: This file has been automatically generated by github.com/aacfactory/gcg. Dont Edit it.
...
func dependencies() (v []service.Service) {
	...
	servicesFilename := filepath.ToSlash(filepath.Join(mf.dir, "service.go"))
	writeErr := os.WriteFile(servicesFilename, []byte(exports), 0600)
	if writeErr != nil {
		err = errors.Warning("forg: modules file write failed").WithCause(writeErr).WithMeta("filename", servicesFilename)
		return
	}
	// fns
	const (
...
`
	...
	writeErr = os.WriteFile(fnsFilename, []byte(strings.ReplaceAll(fns, "#path#", mf.path)), 0600)
	if writeErr != nil {
		err = errors.Warning("forg: modules file write failed").WithCause(writeErr).WithMeta("filename", servicesFilename)
		return
	}
...
package examples`
	...
	err = os.WriteFile(filepath.ToSlash(filepath.Join(dir, "doc.go")), []byte(doc), 0600)
	if err != nil {
		err = errors.Warning("forg: modules file write failed").WithCause(err).WithMeta("filename", filepath.ToSlash(filepath.Join(dir, "doc.go")))
		return
	}
	// hello
	const (
...
`
	...
	err = os.WriteFile(filepath.ToSlash(filepath.Join(dir, "hello.go")), []byte(doc), 0600)
	if err != nil {
		err = errors.Warning("forg: modules file write failed").WithCause(err).WithMeta("filename", filepath.ToSlash(filepath.Join(dir, "hello.go")))
		return
	}
	// fns
	const (
...
`
	...
	err = os.WriteFile(filepath.ToSlash(filepath.Join(dir, "fns.go")), []byte(strings.ReplaceAll(fns, "#path#", mf.path)), 0600)
	if err != nil {
		err = errors.Warning("forg: modules file write failed").WithCause(err).WithMeta("filename", filepath.ToSlash(filepath.Join(dir, "fns.go")))
		return
	}
	return
}
